...
int queuecompare(const void *a, const void *b){
    ...
    if(aa->currentQueue > bb->currentQueue){
        return -1;
    }
    else if (aa->currentQueue < bb->currentQueue){
        return 1;
    }
    else {
...
int processor_2(process *cpuProcesses[processors],int a){
    int i;
    for(i=0; i < processors; i++){
        if(cpuProcesses[i]==NULL || cpuProcesses[i]->currentQueue > a){
            return 1;
        }
    }
    return -1;
...
int processor_1(process *cpuProcesses[processors]){
    int a;
    for (a=0;a<processors;a++){
        if(cpuProcesses[a]==NULL){
            return a;
        }
    }
    return -1;
...
int processorAV(process *cpuProcesses[processors]){
    int a = processor_1(cpuProcesses);
    if(a>=0)
        return 1;
...
int cpucount(process *cpu[numberOfProcesses]){
    int i, counter = 0;
...
void addfront(process *a, process_queue *b){
    process_node *node = createProcessNode(a);
    if(b->front == NULL){
        b->front = b->back = node;
    }
    else {
